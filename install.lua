...
local RunBot = io.open("KAITO", 'w')
RunBot:write([[
#!/usr/bin/env bash
cd $HOME/KAITO
token="]]..database:get(Server_Done.."Token_Write")..[["
rm -fr KAITO.lua
wget "https://raw.githubusercontent.com/TEAM-KAITO/KAITO/main/KAITO.lua"
while(true) do
rm -fr ../.telegram-cli
./tg -s ./KAITO.lua -p PROFILE --bot=$token
done
]])
RunBot:close()
local RunTs = io.open("KIT", 'w')
RunTs:write([[
#!/usr/bin/env bash
cd $HOME/KAITO
while(true) do
rm -fr ../.telegram-cli
screen -S KAITO -X kill
screen -S KAITO ./KAITO
done
]])
RunTs:close()
end
